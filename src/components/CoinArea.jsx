...
const CoinArea = () => {
  ...
  const handleCurrencySelect = (selectedCurrency) => {
    setCurrentCurrency(currencyOptions[selectedCurrency] || currencyOptions.usd);
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-gray-900 via-gray-900/95 to-gray-900/90 text-white px-4 sm:px-[5%] py-6 md:py-10 relative z-0'>
      ...
      {isCurrencyDropdownOpen && (
        <div className='absolute z-50 w-48 mt-2 bg-gray-800/95 backdrop-blur-xl rounded-lg border border-emerald-500/20 shadow-2xl'>
      ...
      <div className='space-y-3 relative z-10'>
        {filteredCryptos.slice(0, 12).map((item) => (
          <Link to={`/crypto/${item.id}`} key={item.id} className='block w-full p-4 bg-gray-800/30 backdrop-blur-md hover:bg-gray-700/40 rounded-xl border border-emerald-500/10 hover:border-cyan-500/30 transition-all duration-300 group'>
            {/* Mobile View */}
            <div className="md:hidden space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 rounded-xl">
                  <span className="text-emerald-400/80 text-sm">#{item.market_cap_rank}</span>
                  ...
                  <div>
                    <p className="font-medium text-gray-100 sm:text-base text-sm">{item.name}</p>
                    <p className="text-xs sm:text-sm text-cyan-400/80 mt-0.5">{item.symbol.toUpperCase()}</p>
                  </div>
                </div>
            ...
            <div className='hidden md:grid grid-cols-5 gap-4 items-center'>
              ...
              <div className='flex items-center gap-3'>
                <img src={item.image} alt={item.name} className='w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 p-0.5'/>
                <div>
              ...
              <div className={`text-center px-2 py-1 rounded-full text-sm lg:text-base ${item.price_change_percentage_24h > 0 ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}>
